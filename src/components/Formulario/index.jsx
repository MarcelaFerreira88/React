...
const Formulario = ({lista,setLista}) => {
  ...
  const criarCartao =(event) => { 
    ...
    const estudante = {   
      nome: nome,
      foto: foto,
      ccomidaQueNaoGosta:comidaNaoGosta,
  ...
  const quantidadeEstudante = useMemo(() => lista.length,[lista]);
  ...
  return (
    <section className="formulario">
      <form onSubmit={criarCartao}>
        ...
        <br />
        <small><i>Atualmente há {quantidadeEstudante} estudantes cadastrados</i></small>
        <CaixaDeTexto
        value={nome}
        label="Nome" 
        placeholder="Digite o seu nome..."
        handleChange={(e) => setNome(e.target.value)} />
        <CaixaDeTexto value={foto} label="Foto" placeholder="Digite a url da sua foto..."
        handleChange={(e) => setFoto(e.target.value)}/>
        <CaixaDeTexto value = {comidaNaoGosta} label="Comida que não gosta"placeholder="Uma comida que não dá!!!"
         handleChange={(e) => setComidaNaoGosta(e.target.value)}/>
        <CaixaDeTexto value = {experienciaMassa} label="Experiência massa" placeholder="Conta ai.."
